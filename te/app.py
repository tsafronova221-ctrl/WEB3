...
def prevent_path_traversal():
    ...
    # === ИСКЛЮЧЕНИЕ: labs/59 - разрешаем все файлы ===
    is_labs_59 = decoded.startswith('instance/labs/59/')
    if is_labs_59:
        print(f"[SECURITY] Исключение для labs/59: пропускаем без проверок расширений")
        # Пропускаем проверки запрещённых паттернов и расширений для labs/59
        print("[BEFORE_REQUEST] Проверка безопасности пройдена (labs/59)")
        print("="*60)
        return
    # =======================================================
